...
local function CreateToggleCheckbox(parent, x, y, text, optionKey)
    ...
    checkbox:SetScript("OnClick", function()
		-- Explicitly cconvert to boolean for Wow 1.12.1
...
local function CreateConfigUI()
    -- NEW CONFIG MENU WITH INDIVIDUAL TICK BOXES
    ...
    masterToggle:SetHeight(24)
     ...
     -- THIS MUST BE INSIDE CreateConfigUI function where masterToggle is created
...
-- Also update the OnClick handlers to be more explicit with boolean conversion
local function CreateToggleCheckbox(parent, x, y, text, optionKey)
    local checkbox = CreateFrame("CheckButton", nil, parent, "UICheckButtonTemplate")
    checkbox:SetPoint("TOPLEFT", x, y)
    checkbox:SetWidth(24)
    ...
    checkbox:SetScript("OnClick", function()
        -- Explicitly convert to boolean (important for WoW 1.12.1)
        local isChecked = checkbox:GetChecked()
        if isChecked then
            ModernMapMarkersDB[optionKey] = true
        else
